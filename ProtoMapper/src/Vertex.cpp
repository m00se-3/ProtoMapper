...
void Buffer2D::Generate(size_t numVertices, size_t numIndices)
{
	if (!_initialized)
	{
		glGenBuffers(1, &_vID);
	...
	// Vertex positions
	glVertexAttribPointer(1u, 2, GL_FLOAT, GL_TRUE, sizeof(Vertex2D), 0);

	// Texture coordinates
	glVertexAttribPointer(2u, 2, GL_FLOAT, GL_TRUE, sizeof(Vertex2D), (const void*)(sizeof(glm::vec2)));

	// Color values
	glVertexAttribPointer(3, 4, GL_FLOAT, GL_TRUE, sizeof(Vertex2D), (const void*)(sizeof(glm::vec2) +sizeof(glm::vec2)));
}

void Buffer2D::WriteData()
{
...
void Buffer2D::SetBufferSize(size_t size) { _vertices.reserve(size); }

void Buffer2D::SetNumberOfIndices(size_t size) { _indices.reserve(size); }
